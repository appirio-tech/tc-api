...
exports.get = function(createDbClient, quoteName) {
    ...
    exports["Query"] = testCase({
        ...
        "orderby()": function(test) {
            ...
            query = client.query().orderby("id ASC, time DESC").sql();
            test.equal(" ORDER BY id ASC, time DESC", query);

            query = client.query().orderby({ "id": true, "time": false }).sql();
            test.equal(" ORDER BY " + quoteName + "id" + quoteName + " ASC," + quoteName + "time" + quoteName + " DESC", query);

            query = client.query().orderby({ "id": "asc", "time": "desc" }).sql();
            test.equal(" ORDER BY " + quoteName + "id" + quoteName + " asc," + quoteName + "time" + quoteName + " desc", query);

            query = client.query().orderby({ "id": true, "time": false }, false).sql();
            test.equal(" ORDER BY id ASC,time DESC", query);

            query = client.query().orderby({ "id": true, "time": false, "CONCAT(first, ' ', last)": { order: false, escape: false } }).sql();
