...
describe('Remove Reviewer API', function () {
    ...
    it("should return required error for empty body or missing username", function (done) {
        assertResponse(adminHeader, {}, "expect_remove_reviewer_with_empty_body", done);
    });
    ...
    it("should return required error for missing categoryId", function (done) {
        assertResponse(adminHeader, _.omit(testBody, 'categoryId'),
            "expect_remove_reviewer_with_missing_categoryId", done);
    });
    ...
    it("should return not found error if not reviewer", function (done) {
        assertErrorResponse(404, adminHeader, _.extend(_.clone(testBody), {username: 'user'}),
            "There is no reviewer with the username:user in category: Architecture", done);
    });

    it("should remove reviewer successfully", function (done) {
        assertResponse(adminHeader, testBody, "expect_remove_reviewer", done);
    });
});
