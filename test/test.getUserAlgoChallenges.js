...
var SQL_DIR = __dirname + '/sqls/getUserAlgoChallenges/';
var API_ENDPOINT = process.env.API_ENDPOINT || 'http://localhost:8080';

describe('Get User Algo Challenges API', function () {
    this.timeout(180000);     // The api with testing remote db could be quit slow

    var msgObj = require('../test/test_files/getUserAlgoChallenges/expected_get_user_algo_challenges_error_message');
    ...
    */
     ...
     * @param {String} userId - the request userId.
     ...
     */
    function createGetRequest(userId, expectStatus, query, cb) {
        request(API_ENDPOINT)
            .get('/v2/user/' + userId + '/challenges/algo' + query)
     ...
     */
    function assertBadResponse(url, query, expectStatus, errorMessage, cb) {
        createGetRequest(url, expectStatus, query, function (err, result) {
            if (!err) {
    ...
    function assertSuccessResponse(url, query, expectedResponse, cb) {
        createGetRequest(url, 200, query, function (err, result) {
    ...
    it('should return bad request. The sortOrder is invalid.', function (done) {
        assertBadResponse('heffan', '?sortOrder=abc&sortColumn=type', 400, msgObj.invalidSortOrder, done);
    });
    ...
    /**
     * The sortColumn is missing.
     */
    it('should return bad request. The sortColumn is missing.', function (done) {
        assertBadResponse('heffan', '?sortOrder=asc', 400, msgObj.missingSortColumn, done);
    });
    ...
    it('should return success results.', function (done) {
        assertSuccessResponse('heffan', '', 'test_files/getUserAlgoChallenges/expected_response_1', done);
    });
    ...
    */
    it('should return success results. The given user have no marathon matches.', function (done) {
        assertSuccessResponse('user10', '', 'test_files/getUserAlgoChallenges/expected_response_2', done);
    });

    /**
     * The given user is not activated.
     */
    it('should return bad request. The given user is not activated.', function (done) {
        assertBadResponse('user2', '', 400, msgObj.inActivatedUser, done);
    });
    ...
    */
    it('should return success results. Test pageIndex filter.', function (done) {
        assertSuccessResponse('heffan', '?pageIndex=2', 'test_files/getUserAlgoChallenges/expected_response_3', done);
    });

    /**
     * Test when pageIndex = -1
     */
    it('should return success results. Test pageIndex=-1.', function (done) {
        assertSuccessResponse('heffan', '?pageIndex=-1', 'test_files/getUserAlgoChallenges/expected_response_7', done);
    });
    ...
    */
    it('should return success results. Test pageSize filter.', function (done) {
        assertSuccessResponse('heffan', '?pageSize=1', 'test_files/getUserAlgoChallenges/expected_response_4', done);
    });
    ...
    */
    it('should return success results. Test sortOrder filter.', function (done) {
        assertSuccessResponse('heffan', '?sortOrder=desc&sortColumn=id', 'test_files/getUserAlgoChallenges/expected_response_5', done);
    });
    ...
    */
    it('should return success results. Test sortColumn filter.', function (done) {
        assertSuccessResponse('heffan', '?sortColumn=numSubmitters', 'test_files/getUserAlgoChallenges/expected_response_6', done);
    });
