...
describe('Create Admin API', function () {
    ...
    it("should return required error for missing username", function (done) {
        assertResponse(adminHeader, {}, "expect_create_admin_with_empty_body", done);
    });
    ...
    it("should return already exist error for exist admin username", function (done) {
        assertErrorResponse(409, adminHeader, {username: 'dok_tester'},
            "User dok_tester has already been added to Admin Group", done);
    });

    it("should create admin successfully", function (done) {
        assertResponse(adminHeader, testBody, "expect_create_admin", done);
    });

    it("should create admin successfully if exist admin resource", function (done) {
        assertResponse(adminHeader, {username: 'wyzmo'}, "expect_create_admin_with_exist_admin_resource", done);
    });

    it("should return admin role exist role if exist admin role", function (done) {
        assertErrorResponse(409, adminHeader, {username: 'cartajs'},
            "User cartajs has already been assigned Admin role", done);
    });
});
