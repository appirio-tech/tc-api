...
describe('Remove Copilot API', function () {
    ...
    it("should return required error for empty body or missing username", function (done) {
        assertResponse(adminHeader, {}, "expect_remove_copilot_with_empty_body", done);
    });
    ...
    it("should return not found error if not exist copilot", function (done) {
        assertErrorResponse(404, adminHeader, _.extend(_.clone(testBody), {username: 'user'}),
            "user is not in the copilot pool", done);
    });


    it("should remove copilot successfully", function (done) {
        assertResponse(adminHeader, testBody, "expect_remove_copilot", done);
    });
});
