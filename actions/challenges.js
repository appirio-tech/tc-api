 ...
 * changes in 1.7:
 * Added logic for getting challenge results by id. This is supported for both studio and software contests.
 */
"use strict";
            ...
            result.results = _.map(res.results, function (el) {
                ...
                if (isStudio) {
                    if (res.restrictions[0].show_submissions) {
                        resEl.submissionDownloadLink = api.configData.designSubmissionLink + el.submission_id;
                        resEl.previewDownloadLink = api.configData.designSubmissionLink + el.submission_id + "&sbt=small";
                    }
                } else {
                    resEl.submissionDownloadLink = api.configData.submissionLink + el.upload_id;
                }
            ...
            if (isStudio) {
                if (res.restrictions[0].show_submissions) {
                    result.finalFixes = _.map(res.finalFixes, function (ff) {
                        return api.configData.designSubmissionLink + ff.submission_id;
                    });
                }
            } else {
                result.finalFixes = _.map(res.finalFixes, function (ff) {
                    return api.configData.finalFixLink + ff.upload_id;
                });
            }
